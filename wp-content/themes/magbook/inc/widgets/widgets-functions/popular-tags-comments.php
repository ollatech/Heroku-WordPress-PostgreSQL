...
class Magbook_tab_Widgets extends WP_Widget {
	...
	public function update( $new_instance, $old_instance ) {
		$instance = array();
		$instance['magbook_popular_posts'] = ( ! empty( $new_instance['magbook_popular_posts'] ) ) ? (int)( $new_instance['magbook_popular_posts'] ) : '';
		$instance['magbook_comments'] = ( ! empty( $new_instance['magbook_comments'] ) ) ? (int)( $new_instance['magbook_comments'] ) : '';
	...
	public function widget( $args, $instance ) {
		...
		<div class="tab-wrapper">
			<ul class="tab-menu">
				<li class="active"><?php esc_html_e( 'Popular', 'magbook' ); ?></li>
				<li><?php esc_html_e( 'Comments', 'magbook' ); ?></li>
				<li><?php esc_html_e( 'Tags', 'magbook' ); ?></li>
			</ul>
									...
									<div class="mb-content">
										...
										<div class="mb-entry-meta">
											<?php
												...
												printf( '<span class="posted-on"><a href="%1$s" title="%2$s"><i class="fa fa-calendar-check-o"></i>%3$s</a></span>',
																esc_url(get_the_permalink()),
																esc_attr( get_the_time() ),
																esc_attr(get_the_time( ' d F Y' ))
															);

											if ( comments_open() ) { ?>
													<span class="comments">
													<?php comments_popup_link( __( '<i class="fa fa-comments-o"></i> No Comments', 'magbook' ), __( '<i class="fa fa-comments-o"></i> 1 Comment', 'magbook' ), __( '<i class="fa fa-comments-o"></i> % Comments', 'magbook' ), '', __( 'Comments Off', 'magbook' ) ); ?> </span>
											<?php } ?>
