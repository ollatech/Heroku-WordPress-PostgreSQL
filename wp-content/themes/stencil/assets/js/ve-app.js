...
function ajax_element(elementType, args, callback) {
	...
	if(stencilDev !== undefined && stencilDev !== null) {
		var stencilAjaxUrl = stencilUrl+'ve/el-'+elementType+'.html';
		jQuery.get(stencilAjaxUrl, data, function(response) {
			callback(response);
		});
	} else {
...
function ajax_element_edit(elementType, args, callback) {
	...
	if(stencilDev !== undefined && stencilDev !== null) {
		var stencilAjaxUrl = stencilUrl+'ve/options.html';
		jQuery.get(stencilAjaxUrl, data, function(response) {
			callback(response);
		});
	} else {
